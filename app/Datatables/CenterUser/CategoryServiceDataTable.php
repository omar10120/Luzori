...
class CategoryServiceDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('translation.name', function ($row) {
                $nameAr = $row->translate('ar')->name ?? '';
                $nameEn = $row->translate('en')->name ?? '';
                $combinedName = trim($nameAr . ' , ' . $nameEn, ' , ');
                return \App\Helpers\MyHelper::truncateWithReadMore($combinedName);
            })
            ->editColumn('parent.name', function ($row) {
                if ($row->parent) {
                    $nameAr = $row->parent->translate('ar')->name ?? '';
                    $nameEn = $row->parent->translate('en')->name ?? '';
                    $combinedName = trim($nameAr . ' , ' . $nameEn, ' , ');
                    return \App\Helpers\MyHelper::truncateWithReadMore($combinedName);
                }
                return '-';
            ...
            ->editColumn('translation.description', function ($row) {
                return \App\Helpers\MyHelper::truncateWithReadMore($row->translation->description ?? '', 100);
            })
            ->rawColumns(['status', 'translation.name', 'parent.name', 'translation.description'], true)
            ->setRowId('id');
    }

    public function query(CategoryService $model): QueryBuilder
    {
        return $model->query()->withTrashed()->with(['translations', 'translation', 'parent.translation'])->orderBy('categories_services.id', 'DESC');
    }
    ...
    public function html(): HtmlBuilder
    {
        ...
        $canCreate = auth('center_user')->check() && auth('center_user')->user()->can('CREATE_CATEGORIES', 'center_api');
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('parent.name')->searchable(false)->title(__('field.parent_category')),
            Column::computed('translation.description')->searchable(true)->title(__('field.description')),
            Column::computed('translation.keywords')->searchable(true)->title(__('field.keywords')),
