...
(function () {
  ...
  function switchMenuIcons(style) {
    if (style === 'system') {
      if (window.matchMedia('(prefers-color-scheme: dark)').matches) {
        style = 'dark';
      } else {
    ...
    const menuIcons = document.querySelectorAll('#layout-menu img[src*="assets/icons/"]');
    
    menuIcons.forEach(function (iconEl) {
      const currentSrc = iconEl.getAttribute('src');
      if (!currentSrc) return;
      
      // Extract icon name from src (e.g., "assets/icons/products.svg" or "assets/icons/products-dark.svg")
      const iconMatch = currentSrc.match(/assets\/icons\/(.+?)\.svg/);
      if (!iconMatch) return;
      
      let iconName = iconMatch[1];
      
      // Remove -dark suffix if present
      if (iconName.endsWith('-dark')) {
        iconName = iconName.replace(/-dark$/, '');
      }
      
      // Add -dark suffix if dark mode, remove if light mode
      if (isDark) {
        if (!iconName.endsWith('-dark')) {
          iconName = iconName + '-dark';
        }
      }
      
      // Update icon src
      const newSrc = assetsPath + 'icons/' + iconName + '.svg';
  ...
  let storedStyle =
    ...
    (window.templateCustomizer?.settings?.defaultStyle ?? 'dark'); //!if there is no Customizer then use default style as dark
  ...
  // Run switchMenuIcons function based on the stored style to update menu icons
  switchMenuIcons(storedStyle);
